...
export async function createAdminSession() {
  ...
  const signed = createHash("sha256").update(`${token}:${process.env.ADMIN_PASSWORD_HASH ?? ""}`).digest("hex");
  (await cookies()).set(COOKIE, `${token}.${signed}`, { httpOnly: true, secure: true, sameSite: "lax", path: "/admin", maxAge: MAX_AGE });
}
...
export async function isAdmin() {
  ...
  const expected = createHash("sha256").update(`${token}:${process.env.ADMIN_PASSWORD_HASH ?? ""}`).digest("hex");
