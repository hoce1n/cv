...
export async function getCvData(locale: Locale) {
  const [
    ...
    db
      .select()
      .from(experiences)
      .orderBy(asc(experiences.displayOrder), asc(experiences.id)),
    db
      .select()
      .from(projects)
    ...
    db
      ...
      .where(eq(localeVisibilityConfig.locale, locale))
      .limit(1),
  ]);
  ...
  const [experienceBulletRows, projectBulletRows] = await Promise.all([
    experienceRows.length
      ? db
          ...
          .where(and(eq(experienceBullets.locale, locale)))
          .orderBy(
            asc(experienceBullets.displayOrder),
            asc(experienceBullets.id)
          )
      : Promise.resolve([]),
    projectRows.length
      ? db
          .select()
          .from(projectBullets)
          .where(and(eq(projectBullets.locale, locale)))
          .orderBy(asc(projectBullets.displayOrder), asc(projectBullets.id))
      : Promise.resolve([]),
  ]);
  ...
  return {
    ...
    experiences: experienceRows.map((experience) => ({
      ...experience,
      bullets: experienceBulletRows.filter(
        (bullet) => bullet.experienceId === experience.id
      ),
    })),
    projects: projectRows.map((project) => ({
      ...project,
      bullets: projectBulletRows.filter(
        (bullet) => bullet.projectId === project.id
      ),
    })),
