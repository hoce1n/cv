...
export default async function AdminPage() {
  ...
  const sections = await Promise.all(
    configs.map(async ([table, label, definition]) => [table, label, await rows(definition)] as const),
  );
  ...
  return (
    <main className="min-h-dvh bg-background text-foreground">
      <header className="sticky top-0 z-10 border-b border-border bg-card/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-4 sm:px-6">
          <div>
            ...
            <h1 className="font-serif text-2xl tracking-[-0.03em]">Admin editor</h1>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href="/cv/fa"
              className="inline-flex h-11 items-center border border-border px-3 text-sm hover:bg-muted"
            >
              Preview FA
            </Link>
            <Link
            ...
            <form action={logout}>
              <button className="inline-flex h-11 items-center border border-primary bg-primary px-3 text-sm text-primary-foreground hover:border-accent hover:bg-accent">
                Sign out
              </button>
            </form>
          </div>
      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-8 lg:grid-cols-[13rem_minmax(0,1fr)] sm:px-6">
        <nav
        ...
        >
          {sections.map(([table, label, data]) => (
            <a
              key={table}
              href={`#${table}`}
              className="inline-flex h-11 shrink-0 items-center border border-border bg-card px-3 text-sm whitespace-nowrap hover:bg-muted lg:flex lg:w-full lg:justify-between"
            >
              <span>{label}</span>
              <span className="ml-3 font-mono text-[11px] text-muted-foreground">{data.length}</span>
            </a>
          ))}
        </nav>
        ...
        <div className="space-y-12">
          <p className="max-w-2xl text-sm leading-7 text-muted-foreground">
            Manage every record directly. Values are validated as JSON against the database schema before writing.
          </p>

          {sections.map(([table, label, data]) => (
            <section key={table} id={table} className="scroll-mt-28 space-y-4">
              <div className="border-b border-border pb-3">
                <h2 className="font-serif text-3xl tracking-[-0.03em]">{label}</h2>
