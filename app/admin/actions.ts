...
const tables: Record<string, any> = {
  personal_info: s.personalInfo,
  summary: s.summaries,
  skills: s.skills,
  experience: s.experiences,
  experience_bullets: s.experienceBullets,
  projects: s.projects,
  project_bullets: s.projectBullets,
  education: s.education,
  languages: s.languages,
  language_details: s.languageDetails,
  locale_visibility_config: s.localeVisibilityConfig,
};
...
export async function login(_: string | undefined, formData: FormData) {
  ...
  if (!verifyAdminPassword(password)) return 'Invalid password';
...
export async function saveRecord(formData: FormData) {
  ...
  if (!allowed.has(parsed.table)) throw new Error('Unknown table');
  const table = tables[parsed.table];
  const data = parseData(parsed.data);
  if (parsed.id)
    await db.update(table).set(data).where(eq(table.id, parsed.id));
  else await db.insert(table).values(data);
...
export async function deleteRecord(formData: FormData) {
  ...
  if (!allowed.has(tableName) || !Number.isInteger(id))
